...
Fixtures represent supported legacy schema ages. The migration must apply cleanly,
refuse new orphan writes, preserve valid rows, register pre-existing dangling attachment
metadata without deleting it, and provide defensive physical-delete cleanup.
"""
...
CREATE TABLE email (email_id INTEGER PRIMARY KEY, user_id INTEGER NOT NULL, account_id INTEGER NOT NULL, subject TEXT, status INTEGER NOT NULL DEFAULT 0, is_del INTEGER NOT NULL DEFAULT 0);
...
def seed(db: sqlite3.Connection) -> None:
    ...
    INSERT INTO email(email_id,user_id,account_id,subject,status,is_del) VALUES (100, 1, 10, 'hello', 0, 0);
...
def run_fixture(name: str, schema: str) -> None:
    ...
    must_abort(db, "INSERT INTO email(email_id,user_id,account_id,subject,status,is_del) VALUES (101, 1, 999, 'bad', 0, 0)", 'integrity:email.owner')
    # 0/0 ownership is only legal for the transient SAVING state and the persisted
    # admin-visible NOONE state. A normal RECEIVE row may never be unowned.
    must_abort(db, "INSERT INTO email(email_id,user_id,account_id,subject,status,is_del) VALUES (102, 0, 0, 'bad-unowned', 0, 0)", 'integrity:email.owner')
    ...
    # Valid owned writes remain possible.
    db.execute("INSERT INTO email(email_id,user_id,account_id,subject,status,is_del) VALUES (101, 1, 10, 'valid', 0, 0)")
    db.execute("INSERT INTO attachments VALUES (1001, 1, 10, 101, 'attachments/valid')")
    db.execute("INSERT INTO star VALUES (2001, 1, 101)")

    # CloudMail's unmatched-recipient path is intentionally unowned while SAVING
    # and after classification as NOONE. Attachments follow the parent 0/0 ownership.
    db.execute("INSERT INTO email(email_id,user_id,account_id,subject,status,is_del) VALUES (102, 0, 0, 'unmatched', 6, 1)")
    db.execute("INSERT INTO attachments VALUES (1002, 0, 0, 102, 'attachments/noone')")
    db.execute("UPDATE email SET status=7, is_del=0 WHERE email_id=102")
    assert db.execute("SELECT status,user_id,account_id FROM email WHERE email_id=102").fetchone() == (7, 0, 0)
    assert db.execute("SELECT user_id,account_id FROM attachments WHERE email_id=102").fetchone() == (0, 0)
    must_abort(db, "UPDATE email SET status=0 WHERE email_id=102", 'integrity:email.owner')
    ...
    # Remove the independent NOONE fixture before testing the user-owned cascade.
    db.execute("DELETE FROM email WHERE email_id=102")
...
def legacy_missing_parent_attachment_is_preserved() -> None:
    db = sqlite3.connect(':memory:')
    db.executescript(BASE)
    seed(db)
    apply_upstream_compatibility_bridge(db)
    db.execute(
        "INSERT INTO attachments VALUES (1999, 1, 10, 9999, 'attachments/legacy-dangling')"
    )
    ...
    assert db.execute(
        "SELECT COUNT(*) FROM attachments WHERE att_id=1999"
    ).fetchone()[0] == 1
    marker = db.execute(
        "SELECT email_id,account_id,user_id,object_key "
        "FROM cloudmail_legacy_attachment_orphan WHERE att_id=1999"
    ).fetchone()
    assert marker == (9999, 10, 1, 'attachments/legacy-dangling')

    must_abort(
        db,
        "INSERT INTO attachments VALUES (2000, 1, 10, 9998, 'attachments/new-dangling')",
        'integrity:attachments.owner',
    )
    db.close()
    print('  PASS legacy-missing-parent-attachment-preserved')


def existing_attachment_owner_mismatch_must_fail() -> None:
    db = sqlite3.connect(':memory:')
    db.executescript(BASE)
    seed(db)
    apply_upstream_compatibility_bridge(db)
    db.execute("UPDATE attachments SET user_id=999 WHERE att_id=1000")
    try:
        for _, migration in MIGRATIONS:
            db.executescript(migration)
    except sqlite3.IntegrityError:
        print('  PASS existing-attachment-owner-mismatch-fails-closed')
        return
    raise AssertionError('migration silently accepted existing attachment owner mismatch')
...
legacy_missing_parent_attachment_is_preserved()
existing_attachment_owner_mismatch_must_fail()
